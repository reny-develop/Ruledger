...
namespace Ruledger.Verify
{
    ...
    /// its parameters reordered cost nothing, and the budget lowered costs only what falls
    /// behind it.
    /// </para>
    /// <para>
    ...
    public class Carrying(Xunit.Abstractions.ITestOutputHelper said)
    {
        ...
        public void HowTheRuleSetIsWrittenIsNotInTheTestDesign(string ruleSet, string renamed, int budget)
        {
            TestDesignDiff diff = Fixture.Apply(Fixture.Walk(ruleSet, budget), renamed);

            said.WriteLine($"{ruleSet} -> {renamed} at budget {budget}: {diff.Common} states in both, "
        ...
        public void ChoicesDoNotPointAtHowTheRuleSetIsWritten(string ruleSet, string renamed)
        {
            ...
            said.WriteLine($"{ruleSet} -> {renamed} at budget 3000: "
        ...
        public void AChoiceThatCouldNotBeCarriedIsReportedWithWhichWayItWentMissing(string ruleSet, string after)
        {
            ...
            said.WriteLine($"{ruleSet} -> {after}: {Carried(diff.After)} carried, "
                + $"{legal} not legal there any more, {reached} not reached inside the budget");
        ...
        // Lowering the budget is the other way a design gets re-derived, and what a smaller
        // walk costs is only the choices it stops in front of. These twenty are made down one
        // line of play, twenty deep, so a budget of a hundred still reaches every one of
        // them; at twelve it does not, and the eight it stops in front of are reported as
        // that rather than dropped.
        [Theory]
        [InlineData(100, 20, 0)]
        [InlineData(12, 12, 8)]
        public void LoweringTheBudgetCostsOnlyTheChoicesItStopsInFrontOf(int budget, int carried, int missed)
        {
            TestDesign shorter = Chosen("reversi", budget);

            int reached = shorter.Edits.Count(edit => edit.Outcome == EditOutcome.NotReached);

            said.WriteLine($"reversi, the committed twenty, budget 3000 lowered to {budget}: "
                + $"{Carried(shorter)} carried, {reached} not reached inside the budget");
        ...
        public void ADifferentSettingGivesTheSameKindOfAnswer()
        {
            TestDesign design = TestDesign.Derive(
                ...
                null,
                new WalkSettings(Budget: 500, ValidationLimit: 200, OutcomeLimit: 8),
                Fixture.Choices("reversi"));

            TestDesignDiff diff = Fixture.Apply(design, "reversi-swapped");

            said.WriteLine("reversi -> reversi-swapped at budget 500, limit 200, outcomes 8: "
        ...
        private static TestDesign Chosen(string ruleSet, int budget) =>
            Fixture.Walk(ruleSet, budget, Fixture.Choices(ruleSet));
    }
}
